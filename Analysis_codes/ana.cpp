...
int main(int argc, char *argv[])
{
  ...
  //input = ee to lightq (NLO) using durham
  if(sample_id==1){
  ...
  //input = ee to lightq (NLO) using anti kt
   if(sample_id==2){
    chain->Add("input/ee_loghtqNLO_antikt_2june.root");
    hstfilename = "output_files/hst_ee_lightjetsNLO_antikt.root";//output histogram file
    sumfilename = "output_files/sum_ee_lightjetsNLO_antikt.txt";//output text file
  }

   //input = ee to bb (NLO) using durham
   if(sample_id==3){
    chain->Add("input/ee_bb_NLO_50k_91GeV_4june.root");
    hstfilename = "output_files/hst_ee_bb_NLO_50k_91GeV_4june.root";//output histogram file
    sumfilename = "output_files/sum_ee_bb_NLO_50k_91GeV_4june.txt";//output text file
  }
