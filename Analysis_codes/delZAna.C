...
void delZAna::Loop()
{
  ...
  for(Int_t entry = 0; entry < numberOfEntries; ++entry){
    ...
    */
    ...
    //##################################################################
    // RecoJets (detector level) - Using direct Delphes Kinematics
    //##################################################################
    ...
    */
    ...
    if (jetreco.size() > 0 && jetgen.size() > 0) {
      ...
      // Calculate the distance STRICTLY between Leading Reco and Leading Gen
      min_dR = delR(jetreco.at(0).v, jetgen.at(0).v);
      // If the leading jets are an exact geometric match (dR < 0.3)
      
      if (min_dR < 0.3) {
	
	//ploting vars for verification
	h.jet[12] -> Fill(min_dR);
	...
	h.recojet[11]->Fill(jetreco.at(0).v.E()); 
        ...
        //-----------------------------------------------------
	//PLOT THE GENJET LUND PLANE FOR GENJETS
        //-----------------------------------------------------
	...
	if (gen_constituents.size() >= 2) {
	  ...
	  //calling our func
        ...
        //-----------------------------------------------------
	//PLOT THE LUND PLANE FOR RECOJETS
        //-----------------------------------------------------
  ...
  
{
  ...
  while (jj.has_parents(j1, j2)) {
    ...
    if(histIndex == 0){
      h.recojet[15]->Fill(log_kt);
      h.recojet[16]->Fill(log_1_over_theta);                
    }
    else if(histIndex == 1){
      h.recojet[17]->Fill(log_kt);
      h.recojet[18]->Fill(log_1_over_theta);                
    }
  ...
  if (histIndex == 0) h.recojet[19]->Fill(split_counter); 
  else if (histIndex == 1) h.recojet[20]->Fill(split_counter); 
...
void delZAna::BookHistograms()
{
  ...
  h.recojet[15] = new TH1F("lund_kt_genjet",                     "Lund Split Momentum for GenJets; ln(k_{t}) [GeV]; Number of Splits",                     50, -8, 6);
  h.recojet[16] = new TH1F("lund_theta_genjet",                  "Lund Split Angle for GenJets #theta; ln(1.0 / #theta); Number of Splits",                50, 0, 7);
  h.recojet[17] = new TH1F("lund_kt_recojet",                     "Lund Split Momentum for RecoJets; ln(k_{t}) [GeV]; Number of Splits",                     50, -8, 6);
  h.recojet[18] = new TH1F("lund_theta_recojet",                  "Lund Split Angle for GenJets #theta; ln(1.0 / #theta); Number of Splits",                50, 0, 7);

  h.recojet[19] = new TH1F("n_splits_genjet",                    "Number of Lund Splits per genJet; Splits; Events",                              30, 0, 30);   

  h.recojet[20] = new TH1F("n_splits_recojet",                    "Number of Lund Splits per recoJet; Splits; Events",                              30, 0, 30);   
  ...
  h.lundPlane[0] = new TH2F("lund_plane_genjet", "Primary Lund Jet Plane for GenJets; ln(1.0 / #theta); ln(k_{t}) [GeV]", 50, 0, 7, 50, -8, 6);
  
  h.lundPlane[1] = new TH2F("lund_plane_recojet", "Primary Lund Jet Plane for RecoJets; ln(1.0 / #theta); ln(k_{t}) [GeV]", 50, 0, 7, 50, -8, 6);


  //After Matching
  
  h.lundPlane[2] = new TH2F("lund_plane_genjet_aftermatching", "Primary Lund Jet Plane for GenJets - After Matching; ln(1.0 / #theta); ln(k_{t}) [GeV]", 50, 0, 7, 50, -8, 6);
  
  h.lundPlane[3] = new TH2F("lund_plane_recojet_aftermatching", "Primary Lund Jet Plane for RecoJets - After Matching; ln(1.0 / #theta); ln(k_{t}) [GeV]", 50, 0, 7, 50, -8, 6);

  h.jet[12]         =   new TH1F("gen0_reco0_dR_aftermatching",        "Delta R between Leading GenJet & RecoJet- After Matching",        40,0,0.4); 

   
  h.recojet[11]         =   new TH1F("recojet_E_aftermatching",          "RecoJet Energy - After Matching",        50,0,100);
  h.jet[13]         =   new TH1F("genjet_E_aftermatching",          "GenJet Energy - After Matching",        50,0,100); 
